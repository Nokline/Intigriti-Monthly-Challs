...
- To make matters worse, sometimes unsanitized JSONP poses no real security threats on the provider itself, but does impact the security of all the clients that depend on it. In our case, google helped us bypass CSP with an unsanitized JSONP endpoint
